...
mkdir $DEBDIR/var
mkdir $DEBDIR/var/www/
mkdir $DEBDIR/var/www/.ssh/
mkdir $DEBDIR/var/www/html/
mkdir $DEBDIR/usr
...
if [ -d $DEBDIR$WEBDIR ]; 
	then 
		cp -avr lanman $DEBDIR$WEBDIR
		chmod -R a+rwX $DEBDIR$WEBDIR/lanman
		chmod a+x $DEBDIR$WEBDIR/lanman
		chmod og-w $DEBDIR$WEBDIR/lanman
		echo "Webpage installed in $DEBDIR$WEBDIR/lanman"
		else 
		echo "Sorry, $DEBDIR$WEBDIR does not exist"
	fi 
if [ -d $DEBDIR$WEBDIRALT ];
	then 
		cp -avr lanman $DEBDIR$WEBDIRALT
		chmod -R a+rwX $DEBDIR$WEBDIRALT/lanman
		chmod a+x $DEBDIR$WEBDIRALT/lanman
		chmod og-w $DEBDIR$WEBDIRALT/lanman
		echo "Webpage installed in $DEBDIR$WEBDIRALT/lanman"
		else 
		echo "Sorry, $DEBDIR$WEBDIRALT does not exist"
	fi 
...
if [ -d $DEBDIR$BINDIR ] ;
    then
        ...
        cp labAuditConfig.ini $DEBDIR$BINDIR/lanManager
